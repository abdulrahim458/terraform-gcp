...
resource "google_compute_instance_template" "web_server" {
  ...
  metadata_startup_script = "sudo apt-get update; sudo apt-get install -yq build-essential apache2; sudo sed -i 's/80/8080/g' /etc/apache2/apache2.conf; sudo service apache2 restart"
}
